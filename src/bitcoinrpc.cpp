...
Value stop(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Stop JurassicCoin server (and possibly override the detachdb config value).");
    // Shutdown will take long enough that the response should get back
    if (params.size() > 0)
        bitdb.SetDetach(params[0].get_bool());
    StartShutdown();
    return "JurassicCoin server stopping";
}
...
void ThreadRPCServer(void* parg)
{
    ...
    RenameThread("jurassiccoin-rpclist");
...
void ThreadRPCServer2(void* parg)
{
    ...
    if (mapArgs["-rpcpassword"] == "")
    {
        ...
        string strWhatAmI = "To use jurassiccoind";
...
void ThreadRPCServer3(void* parg)
{
    ...
    RenameThread("jurassiccoin-rpchand");
